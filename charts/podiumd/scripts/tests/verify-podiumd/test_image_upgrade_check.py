...
def test_check_image_upgrades_splits_own_partner_other_and_never_fails(
    ...
    assert "1/1 own upgradeable" in detail
    assert "0/1 partner-vendor upgradeable" in detail
    assert "0/1 other-vendor upgradeable" in detail
...
def test_check_image_upgrades_partner_upgrade_shown_with_vendor_label(
    ...
    ok, detail = vp.check_image_upgrades(chart_dir, [])
    assert ok is True
    assert "1/1 partner-vendor upgradeable" in detail
    out = capsys.readouterr().out
...
def test_check_image_upgrades_other_vendor_aggregate_line(vp, libimageupgradecheck, tmp_path, monkeypatch, capsys):
    ...
    ok, detail = vp.check_image_upgrades(chart_dir, [])
    assert ok is True
    assert "1/1 other-vendor upgradeable" in detail
    out = capsys.readouterr().out
...
def test_check_image_upgrades_nothing_upgradeable_prints_ok(vp, libimageupgradecheck, tmp_path, monkeypatch, capsys):
    ...
    ok, detail = vp.check_image_upgrades(chart_dir, [])
    assert ok is True
    assert "0/1 own upgradeable" in detail
    assert "0/1 partner-vendor upgradeable" in detail
    assert "0/1 other-vendor upgradeable" in detail
    out = capsys.readouterr().out
...
def test_check_image_upgrades_expired_cache_entry_rechecks(vp, libimageupgradecheck, tmp_path, monkeypatch, capsys):
    ...
    ok, detail = vp.check_image_upgrades(chart_dir, [])
    assert ok is True
    assert "0/1 own upgradeable" in detail  # stale "999" replaced by the fresh (unchanged) result
    out = capsys.readouterr().out
...
def test_check_image_upgrades_heuristic_fallback_for_disabled_component(
    ...
    ok, detail = vp.check_image_upgrades(chart_dir, [])
    assert ok is True
    assert "1/2 own upgradeable" in detail  # frankgateway + apiproxy, only apiproxy has an upgrade
    out = capsys.readouterr().out
