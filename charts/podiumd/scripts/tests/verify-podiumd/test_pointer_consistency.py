...
def test_no_references_is_clean(vp, dirs):
    doc_dir, images_dir = dirs
    doc = doc_dir / "4.8.5-to-4.9.0-upgrade.md"
    doc.write_text("# Upgrade guide\n\nNothing to see here.\n")
    assert vp.check_pointer_consistency(doc, "4.8.5", "4.9.0", doc_dir, images_dir) == []


def test_correct_sibling_reference_passes(vp, dirs):
    doc_dir, images_dir = dirs
...
def test_reference_to_a_different_historical_hop_is_ignored(vp, dirs):
...
def test_stale_baseline_in_sibling_reference_is_flagged(vp, dirs):
    doc_dir, images_dir = dirs
    ...
    issues = vp.check_pointer_consistency(doc, "4.8.5", "4.9.0", doc_dir, images_dir)
    assert issues
    assert all("expected \"4.8.5\"" in i for i in issues)


def test_reference_to_nonexistent_sibling_is_flagged(vp, dirs):
    doc_dir, images_dir = dirs
    doc = doc_dir / "4.8.5-to-4.9.0-upgrade.md"
    doc.write_text("See [`4.8.5-to-4.9.0-values-deltas.md`](4.8.5-to-4.9.0-values-deltas.md).\n")
    issues = vp.check_pointer_consistency(doc, "4.8.5", "4.9.0", doc_dir, images_dir)
...
def test_correct_images_reference_passes(vp, dirs):
    doc_dir, images_dir = dirs
...
def test_images_reference_wrong_version_is_flagged(vp, dirs):
    doc_dir, images_dir = dirs
    doc = doc_dir / "4.8.5-to-4.9.0-upgrade.md"
...
def test_images_reference_to_nonexistent_file_is_flagged(vp, dirs):
    doc_dir, images_dir = dirs
    doc = doc_dir / "4.8.5-to-4.9.0-upgrade.md"
    doc.write_text("See [`images-4.9.0.yaml`](../images/images-4.9.0.yaml).\n")
    issues = vp.check_pointer_consistency(doc, "4.8.5", "4.9.0", doc_dir, images_dir)
