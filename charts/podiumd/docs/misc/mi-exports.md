...
Each CronJob:
1. Reads its target component's existing `<component>` Secret + ConfigMap (DB host, port, name, user, password — same secrets the app pods consume). Both are mounted `optional: true`: a component that exposes no `DB_HOST/DB_NAME/DB_USER/DB_PASSWORD` this way (a .NET app using `ConnectionStrings__*`, or a component that isn't deployed) is treated as **not using PostgreSQL** — the job logs `skip: … no DB_* credentials` and exits green instead of hanging in `CreateContainerConfigError`. A *partial* `DB_*` set still fails the job (real misconfiguration).
...
| Job logs `skip: component <x> exposes no DB_* credentials …` and exits 0 | The component's Secret/ConfigMap carry no `DB_HOST/DB_NAME/DB_USER/DB_PASSWORD` (e.g. a .NET app using `ConnectionStrings__*`, or the component isn't deployed) — the export treats the database as unused | Expected for non-Postgres-convention components (ita, kiss, pabc). If the component *should* export, its DB credentials must be exposed via the `DB_*` convention, or set per-target `secretName`/`configMapName` to resources that carry them. |
| Job pod fails with `partial DB_* credentials for <x> (missing: …)` | Some but not all `DB_*` envs present — typo'd per-target `secretName`/`configMapName`, or a half-migrated component Secret/ConfigMap | Fix the target override or the component's Secret/ConfigMap so all four `DB_*` keys resolve. |
