...
a separate chart too) — see [[objecten-dependencies]] for how PodiumD
currently wires those two APIs as separate subcharts, and
...
3. **Redis DB index** — keep objecten's or objecttypen's; no functional
   difference, purely a consistency/docs call. **Checked upstream — there
   is none.** `docs/manual/migration.rst` (the authoritative migration
   guide) has exactly four sections (intro, version flow, importing
   objecttype data, setup-configuration changes) and mentions nothing about
   Redis, cache, or Celery broker/backend selection. Purely a PodiumD
   decision.
...
Status as of 2026-08-04. **Execution has started**: the full Chart.yaml/
values.yaml/template/docs diff is now drafted as reviewable patch files under
`patches/` at the repo root (one patch per logical change — see that
directory's contents; none applied to the working tree yet, by design, so
diffs stay visible for review before anything is staged/committed).
...
- [x] ~~Draft the actual `Chart.yaml`/`values.yaml`/template diff for the
      `openobject` swap (sections B–D)~~ — **drafted, not yet applied**:
      12 reviewable patch files under `patches/` at the repo root (Chart.yaml,
      values.yaml core restructure, downstream-consumer values, KISS collapse,
      3 template patches, Keycloak template merge, observability/lint-values,
      and 4 docs patches). `charts/podiumd/` itself is untouched — each patch
      was generated by editing the file, capturing `git diff`, then reverting,
      specifically so the diffs stay visible as standalone files for review
      before anything is staged. Apply with `git apply patches/NN-*.patch`
      once reviewed, in numeric order (02 depends on 01's alias having landed
      conceptually, though the patches themselves don't overlap in content).
- [ ] Pin an image digest for `maykinmedia/open-object` via
      `/verify-image-digests` / `/images-manifest` before merging — the
      drafted `patches/02-values-yaml.patch` still carries a placeholder tag
      (`4.1.0`, no digest) with a `TODO` comment.
- [x] ~~Scaffold `docs/_UPGRADE_PATHS/<from>-to-<to>-upgrade.md`~~ — drafted
      as `docs/_UPGRADE_PATHS/4.9.0-to-4.10.0-upgrade.md` (new file, written
      directly rather than as a patch since it doesn't exist in git yet).
      Marked as a draft: the target version number (`4.10.0`) is a
      placeholder pending team confirmation of which release this ships in.
...
- [ ] Inventory per-gemeente overrides for the section-F downstream
      ...
      own item.
- [ ] Write an explicit rollback runbook for the `openobject` cutover
      itself, per `docs/manual/migration.rst` (A.6): if any objecttype
      remains unimported, **the 4.0.0 container fails at startup and blocks
      its own DB schema migrations** — this is a hard failure enforced by
      the container, not just a soft pre-check — and recovery means rolling
      the release back to `objecten` 3.6.1, fixing the remaining
      unimported/stale objecttypes, and retrying. Not yet written anywhere;
      should probably live in the `docs/_UPGRADE_PATHS/...` doc (item above).
