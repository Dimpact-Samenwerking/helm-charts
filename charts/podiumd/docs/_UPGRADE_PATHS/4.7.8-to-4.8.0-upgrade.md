...
form; see [`openbeheer.md`](../apps/openbeheer/openbeheer.md) for the full
enablement checklist. Open Beheer is new in 4.8.0 and ships disabled — enabling it
also requires `configuration.oidcUrl` and `configuration.secrets.keycloak_client_secret`,
without which the deploy succeeds and every login fails.
