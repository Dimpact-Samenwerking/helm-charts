# Upgrade guide: PodiumD 4.6.6 → 4.7.3

> **Consolidated direct-jump guide** for environments on **4.6.6** upgrading
> straight to **4.7.3**. It folds 4.6.7, 4.6.8 and the whole 4.7.x line. See
> [`UPGRADING.md`](../UPGRADING.md) for the full upgrade path. If you prefer
> smaller hops, do [4.6.6 → 4.6.8](upgrade-from-4.6.6-to-4.6.8.md) then
> [4.6.8 → 4.7.3](upgrade-from-4.6.8-to-4.7.3.md) — this guide is the union of
> the two.
>
> **Open Inwoner:** moves from `2.1.1` (at 4.6.6) to the **stable `2.1.2`** and
> stays there through 4.7.3. The `2.1.2-rc1` release candidate (carried briefly
> by 4.6.7 / 4.7.0) is **never** part of this path; do not pin it.
>
...
| Open Inwoner | 2.1.1 → 2.1.2 (stable) | 2.1.3 |
...
> Open Inwoner is `2.1.1` at 4.6.6 and lands on stable `2.1.2` (via 4.6.8). There
> is no rc1 step on this path.
...
The Open Inwoner image is updated from `2.1.1` (4.6.6) to the **stable** `2.1.2` upstream release (promoted at 4.6.8), carrying the DRT-557 fix ([#306](https://github.com/Dimpact-Samenwerking/helm-charts/pull/306)). It stays on stable `2.1.2` through 4.7.3.

> **Do not use `2.1.2-rc1`.** Always pin `openinwoner.image.tag` to the stable `2.1.2`.

**Action required:** none beyond mirroring the `maykinmedia/open-inwoner:2.1.2` tag and digest to ACR.
...
`apiproxy.sslVerifyDepth` (default `6`) renders `proxy_ssl_verify_depth` for every upstream location. nginx default is `1`, which is too shallow for cross-signed government API chains. Override globally or per upstream:
...
### Keycloak 26.5.7 → 26.6.2 (security releases)

Across 4.7.0 → 4.7.2 the Keycloak server and operator images move from `26.5.7` to `26.6.1` (4.7.0) and then to `26.6.2` (4.7.2, security release). The operator chart (Adfinis 1.11.4) is unchanged. Note this also requires the [Keycloak CRD upgrade](#keycloak-crd-upgrade-v2alpha1--v2beta1).

Keycloak 26.6.2 fixes a substantial set of CVEs, including several account-takeover-class issues:
...
### nginx-unprivileged → 1.30.2 (security release)
...
The ZAC application image lands on `4.7.2` (helm chart `1.0.228`). ZAC also switched to native **Gotenberg** (`gotenberg/gotenberg:8.31.0`) for document conversion, replacing `ghcr.io/eugenmayer/kontextwork-converter`; container port `8080` → `3000` (handled in base `values.yaml`). **No action required** beyond the ACR office_converter mirror update noted in [Required manual steps](#before-upgrading).
...
New egress API gateway. **Disabled by default** (`apisix.enabled: false`) — existing environments are unaffected. To opt in, see [`apisix-egress-gateway.md`](../apps/apisix/apisix-egress-gateway.md) for the full setup.

---

## Component changelogs (no action required)

- **Keycloak 26.6.x** — security releases (see CVE list above); endpoints open during initialization (PodiumD already sets `health-enabled: true`); stricter client-URI validation only if the `secure-client-uris` executor is manually enabled.
...
- **ZAC 4.7.x (chart 1.0.228)** — native Gotenberg replaces kontextwork-converter; BPMN process flow sidebar; inbox document deserialization fix.
...
- **Open Inwoner 2.1.2** — stable release (never use `2.1.2-rc1`).

---

## See also

- [`upgrade-from-4.6.6-to-4.6.8.md`](upgrade-from-4.6.6-to-4.6.8.md) + [`upgrade-from-4.6.8-to-4.7.3.md`](upgrade-from-4.6.8-to-4.7.3.md) — the same upgrade as two smaller hops.
...
- Images: [`images-4.6.8.yaml`](../images/images-4.6.8.yaml), [`images-4.7.0.yaml`](../images/images-4.7.0.yaml) … [`images-4.7.3.yaml`](../images/images-4.7.3.yaml).
