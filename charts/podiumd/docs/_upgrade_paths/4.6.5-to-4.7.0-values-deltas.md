# Values deltas — PodiumD 4.6.5 → 4.7.0

Companion to [4.6.8-to-4.7.6-upgrade.md](4.6.8-to-4.7.6-upgrade.md) (consolidated guide covering the 4.7.0 jump). This file lists every value override a gemeente `podiumd.yml` may need to add, change, or remove when moving from chart 4.6.5 to 4.7.0. Application-level changes and migration scripts are documented in the upgrade guide; this file focuses purely on the values surface.

## TL;DR

The **#** column refers to the numbered section below with the full details.

| # | Component | Required action | Type |
|---|-----------|----------------|------|
| 1 | `openarchiefbeheer.configuration.data` (OIDC block) | Migrate `oidc_db_config_admin_auth` to providers/items + options structure | **Required** if openarchiefbeheer enabled |
| 2 | `zac.office_converter.image.repository` | Change `<acr>/office-converter` → `<acr>/gotenberg` | **Required** if repository overridden |
| 2 | `zac.office_converter.image.tag` | `1.8.2` → `8.31.0` | **Required** if tag overridden |
| 2 | `zac.office_converter.containerPort` | Remove override (chart default now `3000`) | **Required** if overridden to `8080` |
| 3 | `openzaak.settings.documentApiBackend` | New optional field, default `filesystem` | Optional |
| 3 | `openzaak.settings.azureBlobStorage` | New optional block | Optional |
| 4 | `openzaak.settings.enableCloudEvents` / `notificationsSource` | New optional fields | Optional |
| 5 | `openarchiefbeheer.settings.postDestructionVisibilityPeriod` | New optional field, default `7` | Optional |
| 5 | `openarchiefbeheer.settings.relatedCountDisabled` | New optional field, default `false` | Optional |
| 6 | `openarchiefbeheer.configuration.data` (`external_registers`) | New optional block (Object API + OpenKlant destruction plugins) | Optional |
| 7 | `openinwoner.image.tag` | Defaults to `2.1.2-rc1` (release candidate, DRT-557). Pin to a stable `2.1.2` before promoting 4.7.0 to production, or roll back to `2.1.1`. | ⚠️ **Before production** |
| 7 | Image tag overrides for openzaak / openformulieren / objecttypen / referentielijsten / omc / zgw-office-addin / keycloak | Drop overrides or bump to chart defaults | Cleanup |
...
### 3. openzaak — Documenten API backend
...
### 5. openarchiefbeheer — Destruction visibility & performance
...
### 6. openarchiefbeheer — External-register destruction plugins
...
| `openinwoner.image.tag` | `2.1.2-rc1` (⚠️ release candidate — see [upgrade guide](4.6.8-to-4.7.6-upgrade.md#component-versions-target-476)) |
...
3. Migration scripts run (see [4.6.8-to-4.7.6-upgrade.md § Migration scripts](4.6.8-to-4.7.6-upgrade.md#migration-scripts)):
