...
Companion to [upgrade-from-4.6.8-to-4.7.6.md](upgrade-from-4.6.8-to-4.7.6.md) (consolidated guide covering the 4.7.0 jump). This file lists every value override a gemeente `podiumd.yml` may need to add, change, or remove when moving from chart 4.6.5 to 4.7.0. Application-level changes and migration scripts are documented in the upgrade guide; this file focuses purely on the values surface.
...
| `openinwoner.image.tag` | `2.1.2-rc1` (⚠️ release candidate — see [upgrade guide](upgrade-from-4.6.8-to-4.7.6.md#component-versions-target-476)) |
...
3. Migration scripts run (see [upgrade-from-4.6.8-to-4.7.6.md § Migration scripts](upgrade-from-4.6.8-to-4.7.6.md#migration-scripts)):
