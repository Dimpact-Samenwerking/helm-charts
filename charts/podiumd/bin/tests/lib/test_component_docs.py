...
def test_ensure_images_manifest_changes_header_noop_when_bare_header_exists(libcomponentdocsheader: ModuleType):
    lines = (
        "# Images new or changed in podiumd 4.9.1 vs 4.9.0.\n#\n# Changes:\n#   1. redis 8.0 -> 8.0.\n"
    ).splitlines(keepends=True)
    original = list(lines)
...
def test_ensure_images_manifest_changes_header_noop_when_counted_header_exists(libcomponentdocsheader: ModuleType):
    lines = (
        "# Images new or changed in podiumd 4.9.1 vs 4.9.0.\n#\n# One change:\n#   1. redis 8.0 -> 8.0.\n"
    ).splitlines(keepends=True)
    original = list(lines)
...
def test_renumber_images_manifest_changes_items_already_correct_is_a_noop(libcomponentdocsheader: ModuleType):
...
def test_renumber_images_manifest_changes_items_no_header_is_a_noop(libcomponentdocsheader: ModuleType):
    lines = ["some: yaml\n"]
...
def test_existing_doc_baselines_groups_by_suffix(
    libcomponentdocs: ModuleType, libcomponentdocsbaselinedocstubs: ModuleType, tmp_path: Path
):
...
def test_existing_doc_baselines_empty_when_no_match(
    libcomponentdocs: ModuleType, libcomponentdocsbaselinedocstubs: ModuleType, tmp_path: Path
):
...
def test_create_missing_docs_creates_all_when_none_exist(
    libcomponentdocs: ModuleType, libcomponentdocsbaselinedocstubs: ModuleType, tmp_path: Path
):
...
def test_create_missing_docs_never_overwrites_existing(
    libcomponentdocs: ModuleType, libcomponentdocsbaselinedocstubs: ModuleType, tmp_path: Path
):
    doc_dir = tmp_path / "docs"
    images_dir = tmp_path / "images"
...
def test_create_missing_docs_nothing_to_do_when_all_exist(
    libcomponentdocs: ModuleType, libcomponentdocsbaselinedocstubs: ModuleType, tmp_path: Path
):
    doc_dir = tmp_path / "docs"
    images_dir = tmp_path / "images"
...
def test_values_delta_section_heading_unresolved_app_version_with_chart(libcomponentdocsdeltas: ModuleType):
...
def test_strip_stale_upgrade_placeholders_never_strips_real_prose_mentioning_todo(libcomponentdocschanges: ModuleType):
    text = (
...
def test_strip_stale_upgrade_placeholders_noop_without_a_changes_heading(libcomponentdocschanges: ModuleType):
...
def test_find_values_delta_section_matches_hand_written_heading(libcomponentdocsdeltas: ModuleType):
...
def test_find_values_delta_section_no_match_returns_none(libcomponentdocsdeltas: ModuleType):
    text = "# Values deltas\n\n## zac 5.0.2 → 5.4.3\n\n- Key `zac.a` was added.\n"
    assert libcomponentdocsdeltas.find_values_delta_section(text, "openformulieren", DEPS) is None


def test_insert_values_delta_section_positions_by_values_yaml_order(libcomponentdocsdeltas: ModuleType):
...
def test_insert_values_delta_section_never_strips_real_prose_mentioning_todo(libcomponentdocsdeltas: ModuleType):
...
def test_strip_stale_values_deltas_todo_stub_never_strips_real_prose_mentioning_todo(
...
def test_strip_stale_values_deltas_todo_stub_noop_without_any_section(libcomponentdocsdeltas: ModuleType):
...
def test_has_real_gemeente_specific_content_true_for_a_real_section(libcomponentdocsdeltas: ModuleType):
    text = GEMEENTE_STUB + "\n## Utrecht (prod)\n\n- Some real finding.\n"
...
def test_append_values_delta_section_body_adds_after_existing_content(libcomponentdocsdeltas: ModuleType):
    text = "# Values deltas\n\n## KISS — required edits\n\nSome prose.\n\n## PABC\n\nOther prose.\n"
...
def test_sync_values_delta_sections_creates_section_only_when_key_lines_exist(
...
def test_prune_empty_values_delta_sections_removes_a_heading_with_nothing_under_it(libcomponentdocsdeltas: ModuleType):
    text = (
        "# Values deltas\n\n"
...
def test_prune_empty_values_delta_sections_never_removes_hand_written_prose(libcomponentdocsdeltas: ModuleType):
...
def test_prune_empty_values_delta_sections_no_sections_is_unchanged(libcomponentdocsdeltas: ModuleType):
    text = "# Values deltas\n\nTODO.\n"
    new_text, removed = libcomponentdocsdeltas.prune_empty_values_delta_sections(text)
    assert removed == []
    assert new_text == text
