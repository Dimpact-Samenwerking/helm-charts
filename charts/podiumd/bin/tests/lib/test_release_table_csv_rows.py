...
def test_read_release_table_reads_rows(tmp_path):
...
def test_read_release_table_rejects_other_header(tmp_path):
    path = tmp_path / "release-table.csv"
...
def test_read_release_table_rejects_short_row(tmp_path):
    path = tmp_path / "release-table.csv"
    path.write_text(",".join(CSV_HEADER) + "\nProduct,Info(NL)\n", encoding="utf-8")
