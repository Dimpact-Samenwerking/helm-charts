"""Loads change-doc-baseline (a hyphenated filename, not importable
...
SCRIPT_PATH = Path(__file__).resolve().parents[2] / "change-doc-baseline"


@pytest.fixture(scope="session")
def cdb():
    loader = SourceFileLoader("change_doc_baseline", str(SCRIPT_PATH))
    spec = importlib.util.spec_from_file_location("change_doc_baseline", SCRIPT_PATH, loader=loader)
...
def stub_update_podiumd_readme(cdb, monkeypatch):
    ...
    target = str(cdb.UPDATE_README_SCRIPT)
