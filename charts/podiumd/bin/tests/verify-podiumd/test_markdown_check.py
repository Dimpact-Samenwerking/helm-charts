...
def make_chart_dir(tmp_path: Path, files=None, upgrade_docs_baseline=None):
...
def test_find_markdown_files_excludes_vendored_charts_dir(libmarkdowncheck: ModuleType, tmp_path: Path):
...
def test_find_markdown_files_excludes_generated_readme(libmarkdowncheck: ModuleType, tmp_path: Path):
...
def test_find_pymarkdown_prefers_repo_root_venv(
    libmarkdowncheck: ModuleType, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    repo_root = tmp_path
...
def test_find_pymarkdown_falls_back_to_path(
    libmarkdowncheck: ModuleType, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    chart_dir = make_chart_dir(tmp_path)
    monkeypatch.setattr(libmarkdowncheck, "find_repo_root", lambda start: None)
    monkeypatch.setattr(
        libmarkdowncheck.shutil, "which", lambda name: "/usr/local/bin/pymarkdown" if name == "pymarkdown" else None
    )

    assert libmarkdowncheck.find_pymarkdown(chart_dir) == "/usr/local/bin/pymarkdown"


def test_find_pymarkdown_returns_none_when_unavailable(
    libmarkdowncheck: ModuleType, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    chart_dir = make_chart_dir(tmp_path)
    monkeypatch.setattr(libmarkdowncheck, "find_repo_root", lambda start: None)
...
def test_pymarkdown_not_installed_fails(
...
def test_no_markdown_files_passes(
...
def test_findings_fail_the_check(
    ...
    monkeypatch.setattr(libmarkdowncheck, "find_pymarkdown", lambda chart_dir: "/usr/local/bin/pymarkdown")
    finding_line = (
        f"{chart_dir / 'docs' / 'foo.md'}:1:1: MD041: First line in file should be a top level "
...
def test_disables_line_length_and_commands_show_output_rules(
    libmarkdowncheck: ModuleType, vp: ModuleType, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    chart_dir = make_chart_dir(tmp_path, files={"docs/foo.md": "# a\n"})
    monkeypatch.setattr(libmarkdowncheck, "find_pymarkdown", lambda chart_dir: "/usr/local/bin/pymarkdown")
...
def test_md024_scoped_to_siblings_only(
    ...
    captured = {}
    monkeypatch.setattr(
        libmarkdowncheck, "run", lambda cmd, **kw: (captured.__setitem__("cmd", cmd), pymarkdown_result("", 0))[1]
    )
...
def test_paths_are_shown_relative_to_chart_dir(
    libmarkdowncheck: ModuleType,
    vp: ModuleType,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(libmarkdowncheck, "find_pymarkdown", lambda chart_dir: "/usr/local/bin/pymarkdown")
    finding_line = (
        f"{chart_dir / 'docs' / 'foo.md'}:1:1: MD041: First line in file should be a top "
