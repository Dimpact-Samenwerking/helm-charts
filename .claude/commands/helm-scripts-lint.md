...
Why this skill exists: a `lib/` refactor of `verify-podiumd` once left ~65 dead imports behind (names re-exported "for compatibility" that the file itself never used). Config lives in `charts/podiumd/bin/pyproject.toml`'s `[tool.ruff]`/`[tool.ruff.lint]` sections (a broad rule set that includes `F` — pyflakes: F401 unused imports, F841 unused locals, etc.) so this class of drift gets caught going forward instead of relying on manual review.
